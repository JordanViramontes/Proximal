...
# variables
@export var player_radius = 10

func _ready() -> void:
	super._ready()
	#ENEMY_STATE += {
		#"run_away":total_states+1,
	#}
	ENEMY_STATE["run_away"] = total_states+1
...
func _on_pathfind_timer_timeout() -> void:
	...
	if current_state != ENEMY_STATE.spawn_edge:
		# if we're in distance, change state 
		if distance_towards_player <= player_radius:
			current_state = ENEMY_STATE.run_away
		else:
			current_state = ENEMY_STATE.roam
	
	#var direction_towards_player = (player.global_transform.origin - global_transform.origin).normalized()
...
func get_target_from_state(state):
	if state == ENEMY_STATE.roam:
		...
		var new_target = global_position - (player_radius * relative_position)
		return new_target
